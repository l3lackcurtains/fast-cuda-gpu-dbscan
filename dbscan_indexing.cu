...
__global__ void DBSCAN(double *dataset, int *cluster, int *seedList,
                       ...
                       int *extraCollision, struct IndexStructure *indexRoot, int * results, int * partition);
...
int main(int argc, char **argv) {
  ...
  /**
   **************************************************************************
   * CUDA Memory allocation for indexing
   **************************************************************************
   */
  ...
  while (!exit) {
    ...
    DBSCAN<<<dim3(THREAD_BLOCKS, 1), dim3(THREAD_COUNT, 1)>>>(
        ...
        d_extraCollision, d_indexRoot, d_results, d_partition);
    gpuErrchk(cudaDeviceSynchronize());
  }
...
__global__ void DBSCAN(double *dataset, int *cluster, int *seedList,
  ...
  for (int i = 0; i < POINTS_SEARCHED; i++) {
    ...
    int searchedPoint = results[chainID * POINTS_SEARCHED + i];

    if(searchedPoint == -1) continue;

    register double comparingPoint[DIMENSION];
    for (int x = 0; x < DIMENSION; x++) {
      comparingPoint[x] = dataset[searchedPoint * DIMENSION + x];
    }
