...
#define EXTRA_COLLISION_SIZE 64
...
__global__ void DBSCAN(double *dataset, int *cluster, int *seedList,
                       ...
                       int *extraCollision, int * partition, int * results, struct IndexStructure *indexRoot);
  ...
  */
  ...
  while (!exit) {
    ...
    DBSCAN<<<dim3(THREAD_BLOCKS, 1), dim3(THREAD_COUNT, 1)>>>(
        ...
        d_extraCollision, d_partition, d_results, d_indexRoot);
    gpuErrchk(cudaDeviceSynchronize());
  }
...
__global__ void DBSCAN(double *dataset, int *cluster, int *seedList,
  ...
  if(threadIdx.x == 0) {
    searchPoints(pointID, chainID, dataset, partition, results, indexRoot);
  }
