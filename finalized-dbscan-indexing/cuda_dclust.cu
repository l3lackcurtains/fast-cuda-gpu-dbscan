 ...
 */
  ...
  clock_t communicationStart, communicationStop, dbscanKernelStart,
      dbscanKernelStop;
  ...
  while (!exit) {
    ...
    int completed = MonitorSeedPoints(
        ...
        d_refillSeedList, d_refillSeedLength, d_collisionMatrix, d_results);
    ...
    communicationTime +=
        (float)(communicationStop - communicationStart) / CLOCKS_PER_SEC;
    ...
    DBSCAN<<<dim3(THREAD_BLOCKS, 1), dim3(THREAD_COUNT, 1)>>>(
        ...
        d_refillSeedLength, d_collisionMatrix, d_results, d_indexBuckets,
        d_indexesStack, d_dataValue, d_upperBounds, d_binWidth);
    gpuErrchk(cudaDeviceSynchronize());

    dbscanKernelStop = clock();
    dbscanKernelTime +=
        (float)(dbscanKernelStop - dbscanKernelStart) / CLOCKS_PER_SEC;
  }
...
__device__ void insertData(int id, double *dataset,
  ...
  while (!found) {
    ...
    int k =
        thrust::upper_bound(
            thrust::device, upperBounds + indexBuckets[currentIndex]->childFrom,
